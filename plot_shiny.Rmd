...
sample_id <- 'SRR11680208'
Gene_Seurat <- readRDS('SRR11680208_done.rds')
```
...
snv_choose <- '9:93121676_A>T'
snv_matrix <- data.frame(fread ('SRR11680208_VAF_matrix.txt'), row.names = 1)
...
choose_seurat_matrix <- left_join(gene_seurat_matrix, snv_choose_matrix, by = "rowname")
...
choose_seurat_matrix$choose <- '0'
choose_seurat_matrix$choose[choose_seurat_matrix[[snv_idx+i]] <= 0.2 & choose_seurat_matrix[[snv_idx+i]] !=0] <- '1'
choose_seurat_matrix$choose[choose_seurat_matrix[[snv_idx+i]] > 0.2 & choose_seurat_matrix[[snv_idx+i]] <=0.4] <- '2'
choose_seurat_matrix$choose[choose_seurat_matrix[[snv_idx+i]] > 0.4 & choose_seurat_matrix[[snv_idx+i]] <= 0.6] <- '3'
choose_seurat_matrix$choose[choose_seurat_matrix[[snv_idx+i]] >  0.6 & choose_seurat_matrix[[snv_idx+i]] <=0.8] <- '4'
choose_seurat_matrix$choose[choose_seurat_matrix[[snv_idx+i]] >  0.8 & choose_seurat_matrix[[snv_idx+i]] <=1] <- '5'
  
ggplot(choose_seurat_matrix , aes(x = UMAP_1, y=UMAP_2)) +
  geom_point(aes(color= as.factor(choose)), alpha = 0.5) +
  geom_text(aes(label = SingleR.cluster.labels), data = class_avg) + 
  labs(title = paste0(names(b)[[snv_idx+i]], '_', sample_id)) +
  ...
  scale_color_manual(values = c('grey','yellow','orange','magenta','red','brown'),
                        labels=c('0/NA', "<0.2","0.2-0.4","0.2-0.4", "0.6-0.8"," >0.8"),
                      name = 'VAF') 
